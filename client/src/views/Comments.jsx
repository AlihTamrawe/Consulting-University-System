...
const Comments = () => {
  ...
  useEffect(() => {
    ...
    loaded2 || axios.get("http://localhost:8000/api/user/" + post.user)
      .then((res) => {
        setUserFull(res.data.user)
        console.log(userFull)
        setLoaded2(true)
      })
      .catch(console.log("user owner"))

    loaded || axios.get(`http://localhost:8000/api/post/${id}/comm`)
      .then(res => { setComments(res.data.allcomments); console.log(res.data.allcomments) })
      .then(setLoaded(true))

      .catch(console.log("comments"))

    loaded3 || axios.get(`http://localhost:8000/api/post/${id}`)
      .then(res => {
        setPost(res.data)
  ...
  const handelcomment = (e) => {
    ...
    const commentFromForm = {
      title: user.user.username + ": " + commentTitle,
      user: user._id,
      post: id,
    };
    console.log(commentFromForm);
    axios.post("http://localhost:8000/api/comm/new", commentFromForm)
      .then((res) => {
      ...
      <Grid container spacing={2} marginTop="2px">
        ...
        <Grid item xs={12} md={8} marginLeft="20rem" alignContent={'right'}>
          {comments.map((comm, id) =>
            <Paper
              elevation={2}
              sx={{ backgroundColor: '#F7F5EB', opacity: '.8', padding: '.9rem', marginBottom: '.9rem', width: '70%', display: 'flex', flexDirection: 'column' }}
              key={id}
            >
              {comm.title}
        ...
        <Grid item xs={12} md={8} marginLeft="20rem" marginBottom={'3rem'}>
          < form onSubmit={handelcomment}>
            ...
            <TextField style={{ width: '60%' }} onChange={e => setcommentTitle(e.target.value)} />
            <Button
              style={{ height: '30px' }}
              startIcon={<SendIcon />}
              size="large"
